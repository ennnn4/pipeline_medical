...
from store.platform_ops import ensure_platform_ops, ensure_platform_admin_user
...
def main():
    ...
    eng = make_engine(url)
    ensure_platform_ops(eng)     # 스키마·함수 적용(멱등·비파괴) — 라이브에 아직 없으면 여기서 생성
    uid = ensure_platform_admin_user(eng, email, pw, name=os.environ.get("PLATFORM_NAME"))
