...
- **로그인**: 첫 실행 시 콘솔에 뜨는 계정(기본 `admin` / `boncure1234`). 팀원은 "계정 만들기"로 추가.
- **화면에서**: 병원 만들기(폼) → 자료 끌어다 놓기 → 주제 넣고 **[대본 만들기]** 버튼 → 대시보드 열기.
- yaml 편집·명령어 필요 없음. (내부적으로 아래 엔진을 그대로 호출)

아래는 **엔진을 터미널로 직접 쓰는** 방법(개발자·자동화용).
