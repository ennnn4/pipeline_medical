...
def _plan_brief_text(plan):
    ...
    L.append(_STYLE_GUIDE)
    return "\n".join(L)


# 대행사 스타일가이드(스요이·유튜브야자)에서 추출한 '대중형 화법 원칙'(브랜드 고유표현 제외, 재사용).
_STYLE_GUIDE = (
    "\n[대중형 화법 가이드 — 논문 설명 톤 금지, 아래를 지켜라]\n"
    "· 훅 먼저: 첫 1~2문장에 핵심. 4가지 중 택1 — ①환자 실제 말 인용(\"이거 진짜 되나요?\") "
    "②도발적 단정(\"이건 하지 마세요\") ③일상 비유로 시작 ④충격 수치 선제시.\n"
    "· 어려운 의학 개념은 '일상 비유'로 번역해 설명(전문용어 나열 금지). 예: 딱딱한 기전 설명 대신 누구나 아는 상황에 빗대기.\n"
    "· 문장은 짧게(대략 15~25자), 해요체(~거든요/~이에요/~하죠/~잖아요). '여러분' 금지, '여러분' 대신 시청자를 직접.\n"
    "· 'A가 아니라 B' 대비를 1회 이상, 중간에 자문자답 질문 1회 이상.\n"
    "· 금지 표현: '대박/99%가 모르는/이것만 하면/무조건/완치'. 클릭베이트·과장·허위 약속 금지.\n"
    "· 의료: 시술 자랑 대신 '원장의 관점·과정'에 초점. 과대광고·전후 과장 금지, 환자 개인정보 보호, 의료광고법 준수.\n"
    "· 단, 톤이 '진지함'이면 위 가벼움 요소는 절제하고 신뢰감 위주로.")
